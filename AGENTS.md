...
> **Production database warning.** `.env` and `.env.prod` point at the **same** Supabase database. `npm run db:push`, `scripts/backfill-*.ts`, and `scripts/fix-*.ts` run against `.env` affect production immediately. There is no separate dev database. Only `.env.staging` is isolated. Read any script that imports `db` before executing it.
...
npm run db:push        # Push schema (targets production, see warning above)
