...
const PUBLICATION_BUILDERS = new Map([["buildPrismaData", "@/services/affairs/proposals"]]);
...
function parse(source: string, file = "fixture.ts"): ts.SourceFile {
  ...
  if (diagnostic) throw new Error(`TypeScript parse error in ${file}: ${ts.flattenDiagnosticMessageText(diagnostic.messageText, "\n")}`);
  return sf;
}
...
function imports(sf: ts.SourceFile, imported: string): Array<{ local: string; module: string }> {
  ...
  for (const statement of sf.statements) {
    ...
    for (const element of named.elements) {
      if ((element.propertyName?.text ?? element.name.text) === imported) result.push({ local: element.name.text, module: statement.moduleSpecifier.text });
    }
  }
  return result;
}
...
function prismaMutation(call: ts.CallExpression, env: Env): { model: string; method: string } | null {
  ...
  return MUTATIONS.has(method) ? { model: chain.at(-2)!, method } : null;
}
...
function jsonParseMember(expr: ts.Expression, env: Env): boolean {
  ...
  return ts.isElementAccessExpression(value) && !!value.argumentExpression && staticString(value.argumentExpression, env) === "parse" && isGlobalJson(value.expression, env);
}
  ...
  for (const statement of sf.statements) {
    if (ts.isVariableStatement(statement)) {
      for (const declaration of statement.declarationList.declarations) {
        ...
        if (hasExport(statement) && ts.isIdentifier(declaration.name) && HTTP_METHODS.has(declaration.name.text)) result.push({ method: declaration.name.text, value: declaration.initializer ?? null, node: declaration });
      }
    }
    if (hasExport(statement) && ts.isFunctionDeclaration(statement) && statement.name && HTTP_METHODS.has(statement.name.text)) result.push({ method: statement.name.text, value: null, node: statement });
...
function analyzeAdmin(source: string, file = "src/app/api/admin/demo/route.ts"): Violation[] {
  ...
  for (const handler of exportedHandlers(sf)) {
    ...
    if (!callee || !ts.isIdentifier(callee) || !wrappers.has(callee.text)) violations.push({ file, line: line(sf, handler.node), message: `${handler.method} is not wrapped by canonical withAdminAuth` });
  }
  return violations;
}
...
function possibleStrings(expr: ts.Expression, env: Env, seen = new Set<string>()): Set<string> | null {
  ...
  if (ts.isPropertyAccessExpression(value)) {
    ...
    if (ts.isIdentifier(base) && /PublicationStatus$/.test(base.text)) return new Set([value.name.text]);
    return null;
  }
  if (ts.isElementAccessExpression(value) && value.argumentExpression) {
    ...
    if (ts.isObjectLiteralExpression(base)) {
      ...
      for (const property of base.properties) {
        ...
        const values = possibleStrings(property.initializer, env, new Set(seen));
        if (!values) return null;
        for (const item of values) strings.add(item);
      }
      return strings;
    }
  }
...
function enclosedByPublishedElse(node: ts.Node, identifier: string): boolean {
  for (let current: ts.Node | undefined = node; current?.parent; current = current.parent) {
    ...
    if (!ts.isBinaryExpression(condition) || ![ts.SyntaxKind.EqualsEqualsEqualsToken, ts.SyntaxKind.EqualsEqualsToken].includes(condition.operatorToken.kind)) continue;
...
function canonicalBuilder(expr: ts.Expression, sf: ts.SourceFile, env: Env): boolean {
  ...
  return imports(sf, "buildPrismaData").some((item) => item.local === local && item.module === PUBLICATION_BUILDERS.get("buildPrismaData"));
}

function publicationDataVerdict(dataExpr: ts.Expression, sf: ts.SourceFile, env: Env, node: ts.Node): "published" | "safe" | "unknown" {
...
function analyzePublication(source: string, file = "src/demo.ts"): Violation[] {
  if (new Set(["src/lib/affairs/publish-guard.ts", "src/lib/measures/transitions.ts"]).has(file)) return [];
  const sf = parse(source, file);
  const env = envFor(sf);
  const violations: Violation[] = [];
  walk(sf, (node) => {
    ...
    for (const key of ["data", "update", "create"]) {
      ...
      const verdict = publicationDataVerdict(data, sf, env, node);
      if (verdict !== "safe") violations.push({ file, line: line(sf, node), message: verdict === "published" ? `direct ${mutation.model} publication write` : "cannot prove publicationStatus non-PUBLISHED" });
    }
  });
  return violations;
}
...
function importerPath(file: string): boolean {
  ...
  return file.startsWith("scripts/") && /^(sync|import|discover|enrich|reconcile)-/.test(path.basename(file));
}
...
function analyzeImporter(source: string, file = "scripts/import-demo.ts"): Violation[] {
  ...
  walk(sf, (node) => {
    ...
    const mutation = prismaMutation(node, env);
    if (!mutation) return;
    if (mutation.model === "affair" && ["create", "createMany"].includes(mutation.method)) violations.push({ file, line: line(sf, node), message: "importer creates Affair directly" });
    ...
    for (const key of ["data", "update", "create"]) {
      ...
      const data = objectExpr(dataExpr, env);
      if (data && objectValue(data, "verifiedAt", env) !== undefined) violations.push({ file, line: line(sf, node), message: "importer writes verifiedAt" });
    }
  });
  return violations;
}

function identifiesPoligraph(value: string | null | undefined): boolean {
...
function httpClientLocals(sf: ts.SourceFile): Set<string> {
  return new Set(imports(sf, "HTTPClient").filter((item) => item.module === "@/lib/api/http-client" || item.module.endsWith("/http-client") || item.module === "./http-client").map((item) => item.local));
}

function analyzeNetworkIdentity(source: string, file = "src/lib/api/demo.ts"): Violation[] {
  const sf = parse(source, file);
  const env = envFor(sf);
  const clients = httpClientLocals(sf);
  const violations: Violation[] = [];
  walk(sf, (node) => {
    if (ts.isIdentifier(node) && ["MEDIAPART_EMAIL", "MEDIAPART_PASSWORD"].includes(node.text)) violations.push({ file, line: line(sf, node), message: "publisher credential reference" });
    if (ts.isStringLiteralLike(node) && node.text.includes("login_check")) violations.push({ file, line: line(sf, node), message: "publisher login endpoint" });

    if (ts.isNewExpression(node) && ts.isIdentifier(node.expression) && clients.has(node.expression.text) && node.arguments?.[0]) {
      const options = objectExpr(node.arguments[0], env);
      if (!options) return;
      const ua = staticProperty(options, "userAgent", env);
      if (ua !== undefined && !identifiesPoligraph(ua)) violations.push({ file, line: line(sf, node), message: "HTTPClient userAgent must identify Poligraph" });
    }
    ...
    if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && node.expression.text === "fetch" && node.arguments[1]) {
      ...
      for (const name of ["User-Agent", "user-agent"]) {
        const raw = objectValue(headers, name, env);
        if (raw === undefined) continue;
        if (
          file === "src/lib/api/http-client.ts" && raw &&
          ts.isPropertyAccessExpression(unwrap(raw)) && unwrap(raw).getText(sf) === "this.options.userAgent"
        ) continue;
        const ua = raw ? staticString(raw, env) : null;
        if (!identifiesPoligraph(ua)) violations.push({ file, line: line(sf, node), message: "fetch User-Agent must identify Poligraph" });
      }
    }
  });
  return violations;
...
function validateHttpClientDefault(source: string): Violation[] {
  ...
  const object = defaults ? objectExpr(defaults, env) : null;
  const ua = object ? staticProperty(object, "userAgent", env) : null;
  return identifiesPoligraph(ua) ? [] : [{ file, line: 1, message: "HTTPClient default userAgent must identify Poligraph" }];
}
...
function analyzePublicEnv(source: string, file = "src/demo.ts"): Violation[] {
  ...
  const check = (name: string | null, node: ts.Node): void => {
    if (name?.startsWith("NEXT_PUBLIC_") && SECRET_MARKERS.some((marker) => name.includes(marker))) violations.push({ file, line: line(sf, node), message: `secret-like public env ${name}` });
  };
  walk(sf, (node) => {
    ...
    if (ts.isElementAccessExpression(node) && node.argumentExpression && isProcessEnv(node.expression, env)) {
      ...
      else if (containsString(node.argumentExpression, "NEXT_PUBLIC_")) violations.push({ file, line: line(sf, node), message: "dynamic NEXT_PUBLIC env name" });
    }
  });
  return violations;
}
...
function validateJsonLd(source: string, file = "src/components/seo/JsonLd.tsx"): Violation[] {
  ...
  const js = ts.transpileModule(`${helper.getText(sf)}\n(globalThis as any).__safe = safeJsonLd;`, { compilerOptions: { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.None } }).outputText;
  ...
  const violations: Violation[] = [];
  if (!serialized || serialized.toLowerCase().includes("</script")) violations.push({ file, line: line(sf, helper), message: "safeJsonLd does not neutralize script closing tag" });
  walk(sf, (node) => {
    ...
    const callee = value && ts.isCallExpression(value) ? unwrap(value.expression) : null;
    if (!callee || !ts.isIdentifier(callee) || callee.text !== "safeJsonLd") violations.push({ file, line: line(sf, node), message: "JSON-LD sink must call unique safeJsonLd" });
  });
  return violations;
}
...
function analyzeHtml(source: string, file: string): Violation[] {
  ...
  const violations: Violation[] = [];
  walk(sf, (node) => {
    if (ts.isJsxAttribute(node) && jsxName(node, sf) === "dangerouslySetInnerHTML") violations.push({ file, line: line(sf, node), message: "dangerouslySetInnerHTML outside canonical JsonLd" });
  });
  return violations;
}

function proposalWhitelistExcludesPublication(source: string): boolean {
  const sf = parse(source, "src/lib/security/schemas/affair-proposal.ts");
  const env = envFor(sf);
  const declaration = env.get("PROPOSABLE_FIELDS");
  if (!declaration) return false;
  let value = unwrap(declaration);
  if (ts.isCallExpression(value) && ts.isPropertyAccessExpression(value.expression) && value.expression.name.text === "freeze" && value.arguments[0]) value = unwrap(value.arguments[0]);
  if (!ts.isArrayLiteralExpression(value)) return false;
  return value.elements.every((element) => !ts.isStringLiteralLike(element) || element.text !== "publicationStatus");
}
...
function repoViolations(): Violation[] {
  ...
  for (const file of [...sourceFiles(path.join(ROOT, "src")), ...sourceFiles(path.join(ROOT, "scripts"))]) {
    ...
    if (fileRel.startsWith("src/app/api/") && !fileRel.startsWith("src/app/api/admin/") && fileRel.endsWith("/route.ts")) violations.push(...analyzeJson(source, fileRel));
    ...
    if (fileRel.startsWith("src/")) {
      const sf = parse(source, fileRel);
      walk(sf, (node) => { if (ts.isIdentifier(node) && node.text === "$executeRawUnsafe") violations.push({ file: fileRel, line: line(sf, node), message: "$executeRawUnsafe forbidden" }); });
    }
  }
  violations.push(...validateHttpClientDefault(fs.readFileSync(path.join(ROOT, "src/lib/api/http-client.ts"), "utf8")));
  if (!proposalWhitelistExcludesPublication(fs.readFileSync(path.join(ROOT, "src/lib/security/schemas/affair-proposal.ts"), "utf8"))) violations.push({ file: "src/lib/security/schemas/affair-proposal.ts", line: 1, message: "proposal whitelist must exclude publicationStatus" });
  fs.readFileSync(path.join(ROOT, ".env.example"), "utf8").split(/\r?\n/).forEach((raw, index) => {
    const value = raw.trim();
    if (!value || value.startsWith("#") || !value.includes("=")) return;
    const name = value.split("=", 1)[0]!.trim();
    if (name.startsWith("NEXT_PUBLIC_") && SECRET_MARKERS.some((marker) => name.includes(marker))) violations.push({ file: ".env.example", line: index + 1, message: `secret-like public env ${name}` });
  });
  return violations;
}
...
describe("CI-01 critical guard contracts", () => {
  ...
  it("checks actual outbound crawler identity case-insensitively", () => {
    const imported = `import {HTTPClient} from "@/lib/api/http-client";`;
    expect(analyzeNetworkIdentity(`${imported} new HTTPClient({userAgent:"Mozilla/"+"5.0"})`)).not.toEqual([]);
    expect(analyzeNetworkIdentity(`${imported} new HTTPClient({userAgent:"PoliGraph/1.0"})`)).toEqual([]);
    expect(analyzeNetworkIdentity(`fetch(url,{headers:{"User-Agent":"curl/8"}})`)).not.toEqual([]);
    expect(analyzeNetworkIdentity(`fetch(url,{headers:{"User-Agent":"poligraph-smoke"}})`)).toEqual([]);
